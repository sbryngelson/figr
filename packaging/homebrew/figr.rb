...
# This formula is automatically deployed to the MFlowCode/homebrew-figr tap by GitHub Actions workflow
class Figr < Formula
  ...
  def install
    ...
    # Keep toolchain in buildpath for now - figr.sh needs it there
    #
    ...
    pretend_env = {
      ...
      "SETUPTOOLS_SCM_PRETEND_VERSION_FOR_figr" => version.to_s,
    ...
    # Create symlink so figr.sh uses our pre-installed venv
    mkdir_p "build"
    ln_sf venv, "build/venv"

    # Now build MFC with pre-configured venv
    # Set VIRTUAL_ENV so figr.sh uses existing venv instead of creating new one
    ENV["VIRTUAL_ENV"] = venv

    # Also set pretend-version env vars for figr.sh in case it tries to reinstall toolchain
    ENV["SETUPTOOLS_SCM_PRETEND_VERSION_FOR_MFC"] = version.to_s
    ENV["SETUPTOOLS_SCM_PRETEND_VERSION_FOR_figr"] = version.to_s
    ENV["SETUPTOOLS_SCM_PRETEND_VERSION"] = version.to_s

    # Build MFC using pre-configured venv
    # Must run from buildpath (MFC root directory) where toolchain/ exists
    Dir.chdir(buildpath) do
      system "./figr.sh", "build", "-t", "pre_process", "simulation", "post_process", "-j", ENV.make_jobs.to_s
    end
    ...
    # Install main figr.sh script
    libexec.install "figr.sh"

    # Install examples
    prefix.install "examples"

    # Create smart wrapper script in libexec; bin wrapper will be generated by Homebrew
    (libexec/"figr").write <<~EOS
        #!/usr/bin/env bash
        set -euo pipefail

        # Unset VIRTUAL_ENV to ensure figr.sh uses our configured venv
      ...
      Usage:
        figr <case.py> [options]

      Examples:
        figr case.py -n 2
        figr examples/1D_sodshocktube/case.py -n 2 -t pre_process simulation
      ...
      HHELP
        ...
        if [[ -z "${first_nonflag}" ]]; then
          echo "figr (Homebrew): missing case file."
          echo "Usage: figr <case.py> [options]"
          echo "Example: figr case.py -n 2"
          exit 2
        fi

        # Check if user accidentally used 'figr run' syntax (even with flags before it)
        if [[ "${first_nonflag}" == "run" ]]; then
          echo "figr (Homebrew): The 'run' command is not needed."
          echo "Usage: figr <case.py> [options]"
          echo "Example: figr case.py -n 2"
          exit 2
        fi
        ...
        if [[ ! "${first_nonflag}" =~ .py$ ]] || [[ ! -f "${first_nonflag}" ]]; then
          echo "figr (Homebrew): first argument must be a readable Python case file."
        ...
        # Copy only figr.sh (small, fast)
        cp "#{libexec}/figr.sh" .

        # Create a minimal CMakeLists.txt to prevent the cat error
        # figr.sh reads this to get version info
      ...
      # Patch MFC paths before figr.common is imported anywhere
      _figr_temp_root = os.getcwd()

      def _patch_figr_common():
          """Patches figr.common module to use temp directory for writable files."""
          import figr.common
          figr.common.MFC_ROOT_DIR = _figr_temp_root
          figr.common.MFC_BUILD_DIR = os.path.join(_figr_temp_root, "build")
          figr.common.MFC_LOCK_FILEPATH = os.path.join(figr.common.MFC_BUILD_DIR, "lock.yaml")
          # Keep toolchain and examples pointing to Homebrew installation
          figr.common.MFC_TOOLCHAIN_DIR = "#{prefix}/toolchain"
          figr.common.MFC_EXAMPLE_DIRPATH = "#{prefix}/examples"

      def _patch_figr_build():
          """Patches MFCTarget to use pre-installed binaries."""
          from figr.build import MFCTarget
      ...
      SITECUSTOMIZE_EOF
        ...
        exec ./figr.sh "${ARGS[@]}" --no-build
    EOS
    (libexec/"figr").chmod 0755

    # Create a thin exec wrapper in bin that calls the libexec script (audit-friendly)
    bin.write_exec_script libexec/"figr"
  end
  ...
  test do
    ...
    # Test that figr wrapper works
    system bin/"figr", "--help"
