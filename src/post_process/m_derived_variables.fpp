...
module m_derived_variables
    ...
    private; public :: s_initialize_derived_variables_module, s_derive_specific_heat_ratio, s_derive_liquid_stiffness, &
        & s_derive_sound_speed, s_derive_flux_limiter, s_derive_vorticity_component, s_derive_qm, &
...
contains
    ...
    subroutine s_derive_sound_speed(q_prim_vf, q_sf)
        ...
        do k = -offset_z%beg, p + offset_z%end
            do j = -offset_y%beg, n + offset_y%end
                do i = -offset_x%beg, m + offset_x%end
                    q_sf(i, j, k) = (((gamma_sf(i, j, k) + 1._wp)*q_prim_vf(E_idx)%sf(i, j, k) + pi_inf_sf(i, j, k))/(gamma_sf(i, &
                         & j, k)*rho_sf(i, j, k)))
    ...
    impure subroutine s_derive_numerical_schlieren_function(q_cons_vf, q_sf)
        ...
        ! The form of the numerical Schlieren function depends on the choice of the multicomponent flow model. For the volume
        ! fraction model, the amplitude of the exponential's inside is modulated with respect to the identity of the fluid in which
        ! the function is evaluated. For more information, refer to Marquina and Mulet (2003).
