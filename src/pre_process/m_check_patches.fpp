...
contains
            ...
            if (i <= num_patches) then
                ...
                if (patch_icpp(i)%geometry == 1) then
                    ...
                    call s_prohibit_abort("Invalid patch geometry number", &
                                          & "patch_icpp(" // trim(iStr) // ")%geometry " // "must be between 1 and 20")
                end if
            else
                @:PROHIBIT(patch_icpp(i)%geometry /= dflt_int, &
    ...
    impure subroutine s_check_active_patch_primitive_variables(patch_id)
        ...
        @:PROHIBIT(n == 0 .and. (.not. f_is_default(patch_icpp(patch_id)%vel(2))) .and. (.not. f_approx_equal(patch_icpp(patch_id) &
                   & %vel(2), 0._wp)), "Patch " // trim(iStr) // ": vel(2) must not be set when n = 0")
        ...
        @:PROHIBIT(p == 0 .and. (.not. f_is_default(patch_icpp(patch_id)%vel(3))) .and. (.not. f_approx_equal(patch_icpp(patch_id) &
                   & %vel(3), 0._wp)), "Patch " // trim(iStr) // ": vel(3) must not be set when p = 0")
