...
module m_data_output
    ...
    abstract interface
        ...
        impure subroutine s_write_abstract_data_files(q_cons_vf, q_prim_vf, bc_type)
            ...
            import :: scalar_field, integer_field, sys_size, m, n, p, num_dims
        ...
        if (n == 0 .and. p == 0) then
            do i = 1, sys_size
                ...
                do j = 0, m
                    ...
                    if (((i >= cont_idx%beg) .and. (i <= cont_idx%end)) .or. ((i >= adv_idx%beg) .and. (i <= adv_idx%end))) then
                        ...
                        call s_compute_pressure(q_cons_vf(E_idx)%sf(j, 0, 0), q_cons_vf(alf_idx)%sf(j, 0, 0), &
                                                & 0.5_wp*(q_cons_vf(mom_idx%beg)%sf(j, 0, 0)**2._wp)/rho, pi_inf, gamma, rho, qv, &
                                                & pres)
